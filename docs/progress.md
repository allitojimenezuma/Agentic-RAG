...
- [x] [COMPLETED] T3: Build `frontend/app.py` Streamlit shell — cached `build_query_agent`, `session_state` thread_id + messages, `st.chat_input`, live tool chips + streamed answer tokens via `stream_query`, structured `QueryAnswer` final render, "New chat" reset `frontend/app.py`
