...
def build_query_agent(settings) -> object:
    ...
    Navigation uses ONE consolidated read-only tool, ``wiki_command`` (pinned
    grammar: scan/search/read/links/match/health, joinable with &&). There is
    NO finalization tool: the response is auto-built from the model's final
    message by ``build_final_answer``, which extracts ``[[Page]]`` links as
    citations and validates them against the turn's ``NavCapture``
    (cite-or-die). A fresh ``NavCapture`` is registered per invocation so every
    cited slug must be a page navigated this turn. The current wiki index is
    injected into the system prompt so the model already knows which pages
    exist without needing an extra tool call.
